    ...
    def process(self, events):
        for e in events:
            ...
            if e.event_type == (*self._gw_prefix, 'gateway', 'running'):
                if payload is False:
                    yield _register_event(('gateway', 'gateway',
                                           'device', 'device',
                                           'system', 'enable'), True)
                ...
                if payload['request_id'] == self._create_model_request_id:
                    ...
                    for bus_id in bus_ids:
                        yield _register_event(('estimation', bus_id, 'v'),
                                              result['vm_pu'][bus_id])
                        yield _register_event(('estimation', bus_id, 'va'),
                                              result['va_degree'][bus_id])
                        yield _register_event(('estimation', bus_id, 'p'),
                                              result['p_mw'][bus_id])
                        yield _register_event(('estimation', bus_id, 'q'),
...
class ModuleSession(common.ModuleSession):
    ...
    async def process(self, changes):
        return [self._engine.create_process_event(self._source, e)
                for e in self._module.process(changes)]
